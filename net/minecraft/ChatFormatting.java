...
public enum ChatFormatting {
    ...
    public static String getLastColors(final String string) {
        final StringBuilder stringBuilder2 = new StringBuilder();
        int integer3 = -1;
        final int integer4 = string.length();
        while ((integer3 = string.indexOf(167, integer3 + 1)) != -1) {
            if (integer3 < integer4 - 1) {
                final ChatFormatting c5 = getByCode(string.charAt(integer3 + 1));
                if (c5 == null) {
                    continue;
                }
                if (c5.shouldReset()) {
    ...
    public static ChatFormatting getById(final int integer) {
        ...
        for (final ChatFormatting c5 : values()) {
            if (c5.getId() == integer) {
                return c5;
            }
        }
    ...
    public static ChatFormatting getByCode(final char character) {
        ...
        for (final ChatFormatting c6 : values()) {
            if (c6.code == character2) {
                return c6;
            }
        }
        return null;
    ...
    public static Collection<String> getNames(final boolean boolean1, final boolean boolean2) {
        ...
        for (final ChatFormatting c7 : values()) {
            if (!c7.isColor() || boolean1) {
                if (!c7.isFormat() || boolean2) {
                    list3.add(c7.getName());
                }
            }
        }
    ...
    static {
        FORMATTING_BY_NAME = (Map)Arrays.stream((Object[])values()).collect(Collectors.toMap(c -> cleanName(c.name), c -> c));
