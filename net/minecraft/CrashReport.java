...
public class CrashReport {
    ...
    public void getDetails(final StringBuilder stringBuilder) {
        ...
        for (final CrashReportCategory e4 : this.details) {
            e4.getDetails(stringBuilder);
            stringBuilder.append("\n\n");
        }
    ...
    public CrashReportCategory addCategory(final String string, final int integer) {
        final CrashReportCategory e4 = new CrashReportCategory(this, string);
        if (this.trackingStackTrace) {
            final int integer2 = e4.fillInStackTrace(integer);
            ...
            this.trackingStackTrace = e4.validateStackTrace(stackTraceElement7, stackTraceElement8);
            if (integer2 > 0 && !this.details.isEmpty()) {
                final CrashReportCategory e5 = (CrashReportCategory)this.details.get(this.details.size() - 1);
                e5.trimStacktrace(integer2);
            }
    ...
    public static CrashReport forThrowable(Throwable throwable, final String string) {
        ...
        if (throwable instanceof ReportedException) {
            d3 = ((ReportedException)throwable).getReport();
        }
        else {
            d3 = new CrashReport(string, throwable);
        }
        return d3;
    }
