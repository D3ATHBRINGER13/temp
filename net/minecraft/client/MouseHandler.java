...
public class MouseHandler {
    ...
    public MouseHandler(final Minecraft cyc) {
        this.activeButton = -1;
    ...
    private void onPress(final long long1, int integer2, final int integer3, final int integer4) {
        ...
        if (this.minecraft.overlay == null) {
            if (this.minecraft.screen == null) {
                ...
                final double double10 = this.xpos * this.minecraft.window.getGuiScaledWidth() / this.minecraft.window.getScreenWidth();
                final double double11 = this.ypos * this.minecraft.window.getGuiScaledHeight() / this.minecraft.window.getScreenHeight();
                if (boolean7) {
    ...
    private void onScroll(final long long1, final double double2, final double double3) {
        if (long1 == Minecraft.getInstance().window.getWindow()) {
            ...
            if (this.minecraft.overlay == null) {
                if (this.minecraft.screen != null) {
                    final double double5 = this.xpos * this.minecraft.window.getGuiScaledWidth() / this.minecraft.window.getScreenWidth();
                    final double double6 = this.ypos * this.minecraft.window.getGuiScaledHeight() / this.minecraft.window.getScreenHeight();
    ...
    public void setup(final long long1) {
        InputConstants.setupMouseCallbacks(long1, this::onMove, this::onPress, this::onScroll);
    }
    ...
    private void onMove(final long long1, final double double2, final double double3) {
        ...
        final GuiEventListener dae8 = this.minecraft.screen;
        if (dae8 != null && this.minecraft.overlay == null) {
            final double double4 = double2 * this.minecraft.window.getGuiScaledWidth() / this.minecraft.window.getScreenWidth();
            final double double5 = double3 * this.minecraft.window.getGuiScaledHeight() / this.minecraft.window.getScreenHeight();
            Screen.wrapScreenError(() -> dae8.mouseMoved(double4, double5), "mouseMoved event handler", dae8.getClass().getCanonicalName());
            if (this.activeButton != -1 && this.mousePressedTime > 0.0) {
                final double double6 = (double2 - this.xpos) * this.minecraft.window.getGuiScaledWidth() / this.minecraft.window.getScreenWidth();
                final double double7 = (double3 - this.ypos) * this.minecraft.window.getGuiScaledHeight() / this.minecraft.window.getScreenHeight();
                Screen.wrapScreenError(() -> dae8.mouseDragged(double4, double5, this.activeButton, double6, double7), "mouseDragged event handler", dae8.getClass().getCanonicalName());
            }
        }
