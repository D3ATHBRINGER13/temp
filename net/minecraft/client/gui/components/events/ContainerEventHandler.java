...
public interface ContainerEventHandler extends GuiEventListener {
    ...
    default Optional<GuiEventListener> getChildAt(final double double1, final double double2) {
        for (final GuiEventListener dae7 : this.children()) {
            if (dae7.isMouseOver(double1, double2)) {
                return (Optional<GuiEventListener>)Optional.of(dae7);
            }
        }
    ...
    default boolean mouseClicked(final double double1, final double double2, final int integer) {
        for (final GuiEventListener dae8 : this.children()) {
            if (dae8.mouseClicked(double1, double2, integer)) {
                this.setFocused(dae8);
                if (integer == 0) {
    ...
    default boolean mouseReleased(final double double1, final double double2, final int integer) {
        ...
        return this.getChildAt(double1, double2).filter(dae -> dae.mouseReleased(double1, double2, integer)).isPresent();
    }
    ...
    default boolean mouseScrolled(final double double1, final double double2, final double double3) {
        return this.getChildAt(double1, double2).filter(dae -> dae.mouseScrolled(double1, double2, double3)).isPresent();
    }
    ...
    void setFocused(@Nullable final GuiEventListener dae);
    
    default void setInitialFocus(@Nullable final GuiEventListener dae) {
        this.setFocused(dae);
    }
    
    default void magicalSpecialHackyFocus(@Nullable final GuiEventListener dae) {
        this.setFocused(dae);
    }
    
    default boolean changeFocus(final boolean boolean1) {
        final GuiEventListener dae3 = this.getFocused();
        final boolean boolean2 = dae3 != null;
        if (boolean2 && dae3.changeFocus(boolean1)) {
            return true;
        }
        final List<? extends GuiEventListener> list5 = this.children();
        final int integer7 = list5.indexOf(dae3);
        int integer8;
        ...
        while (booleanSupplier9.getAsBoolean()) {
            final GuiEventListener dae4 = (GuiEventListener)supplier10.get();
            if (dae4.changeFocus(boolean1)) {
                this.setFocused(dae4);
                return true;
            }
        }
