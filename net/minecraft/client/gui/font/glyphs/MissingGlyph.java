...
public enum MissingGlyph implements RawGlyph {
    ...
    public void upload(final int integer1, final int integer2) {
        MissingGlyph.IMAGE_DATA.upload(0, integer1, integer2, false);
    }
    ...
    static {
        IMAGE_DATA = Util.<NativeImage>make(new NativeImage(NativeImage.Format.RGBA, 5, 8, false), (java.util.function.Consumer<NativeImage>)(cuj -> {
            for (int integer2 = 0; integer2 < 8; ++integer2) {
                for (int integer3 = 0; integer3 < 5; ++integer3) {
                    ...
                    cuj.setPixelRGBA(integer3, integer2, boolean4 ? -1 : 0);
                }
            }
            cuj.untrack();
        }));
