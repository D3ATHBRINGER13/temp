...
public enum GlyphProviderBuilderType {
    ...
    public static GlyphProviderBuilderType byName(final String string) {
        final GlyphProviderBuilderType dbc2 = (GlyphProviderBuilderType)GlyphProviderBuilderType.BY_NAME.get(string);
        if (dbc2 == null) {
    ...
    static {
        BY_NAME = Util.<Map>make((Map)Maps.newHashMap(), (java.util.function.Consumer<Map>)(hashMap -> {
            for (final GlyphProviderBuilderType dbc5 : values()) {
                hashMap.put(dbc5.name, dbc5);
            }
        }));
    }
}
