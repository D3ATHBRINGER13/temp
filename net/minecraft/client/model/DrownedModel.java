...
public class DrownedModel<T extends Zombie> extends ZombieModel<T> {
    public DrownedModel(final float float1, final float float2, final int integer3, final int integer4) {
        ...
        (this.rightArm = new ModelPart(this, 32, 48)).addBox(-3.0f, -2.0f, -2.0f, 4, 12, 4, float1);
        this.rightArm.setPos(-5.0f, 2.0f + float2, 0.0f);
        (this.rightLeg = new ModelPart(this, 16, 48)).addBox(-2.0f, 0.0f, -2.0f, 4, 12, 4, float1);
    ...
    @Override
    public void prepareMobModel(final T avm, final float float2, final float float3, final float float4) {
        this.rightArmPose = ArmPose.EMPTY;
        this.leftArmPose = ArmPose.EMPTY;
        final ItemStack bcj6 = avm.getItemInHand(InteractionHand.MAIN_HAND);
        if (bcj6.getItem() == Items.TRIDENT && avm.isAggressive()) {
            if (avm.getMainArm() == HumanoidArm.RIGHT) {
        ...
        super.prepareMobModel(avm, float2, float3, float4);
    }
    ...
    public void setupAnim(final T avm, final float float2, final float float3, final float float4, final float float5, final float float6, final float float7) {
        ...
        if (this.swimAmount > 0.0f) {
            this.rightArm.xRot = this.rotlerpRad(this.rightArm.xRot, -2.5132742f, this.swimAmount) + this.swimAmount * 0.35f * Mth.sin(0.1f * float4);
            this.leftArm.xRot = this.rotlerpRad(this.leftArm.xRot, -2.5132742f, this.swimAmount) - this.swimAmount * 0.35f * Mth.sin(0.1f * float4);
