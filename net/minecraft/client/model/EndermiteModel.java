...
public class EndermiteModel<T extends Entity> extends EntityModel<T> {
    ...
    public EndermiteModel() {
        ...
        for (int integer3 = 0; integer3 < this.bodyParts.length; ++integer3) {
            (this.bodyParts[integer3] = new ModelPart(this, EndermiteModel.BODY_TEXS[integer3][0], EndermiteModel.BODY_TEXS[integer3][1])).addBox(EndermiteModel.BODY_SIZES[integer3][0] * -0.5f, 0.0f, EndermiteModel.BODY_SIZES[integer3][2] * -0.5f, EndermiteModel.BODY_SIZES[integer3][0], EndermiteModel.BODY_SIZES[integer3][1], EndermiteModel.BODY_SIZES[integer3][2]);
    ...
    public void setupAnim(final T aio, final float float2, final float float3, final float float4, final float float5, final float float6, final float float7) {
        for (int integer9 = 0; integer9 < this.bodyParts.length; ++integer9) {
            this.bodyParts[integer9].yRot = Mth.cos(float4 * 0.9f + integer9 * 0.15f * 3.1415927f) * 3.1415927f * 0.01f * (1 + Math.abs(integer9 - 2));
            this.bodyParts[integer9].x = Mth.sin(float4 * 0.9f + integer9 * 0.15f * 3.1415927f) * 3.1415927f * 0.1f * Math.abs(integer9 - 2);
        }
    }
