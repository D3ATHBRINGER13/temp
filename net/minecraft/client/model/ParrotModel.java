...
public class ParrotModel extends EntityModel<Parrot> {
    ...
    public ParrotModel() {
        this.texWidth = 32;
        this.texHeight = 32;
        (this.body = new ModelPart(this, 2, 8)).addBox(-1.5f, 0.0f, -1.5f, 3, 6, 3);
        this.body.setPos(0.0f, 16.5f, -3.0f);
        (this.tail = new ModelPart(this, 22, 1)).addBox(-1.5f, -1.0f, -1.0f, 3, 4, 1);
        this.tail.setPos(0.0f, 21.07f, 1.16f);
        (this.wingLeft = new ModelPart(this, 19, 8)).addBox(-0.5f, 0.0f, -1.5f, 1, 5, 3);
        this.wingLeft.setPos(1.5f, 16.94f, -2.76f);
        (this.wingRight = new ModelPart(this, 19, 8)).addBox(-0.5f, 0.0f, -1.5f, 1, 5, 3);
        this.wingRight.setPos(-1.5f, 16.94f, -2.76f);
        (this.head = new ModelPart(this, 2, 2)).addBox(-1.0f, -1.5f, -1.0f, 2, 3, 2);
        this.head.setPos(0.0f, 15.69f, -2.76f);
        (this.head2 = new ModelPart(this, 10, 0)).addBox(-1.0f, -0.5f, -2.0f, 2, 1, 4);
        this.head2.setPos(0.0f, -2.0f, -1.0f);
        this.head.addChild(this.head2);
        (this.beak1 = new ModelPart(this, 11, 7)).addBox(-0.5f, -1.0f, -0.5f, 1, 2, 1);
        this.beak1.setPos(0.0f, -0.5f, -1.5f);
        this.head.addChild(this.beak1);
        (this.beak2 = new ModelPart(this, 16, 7)).addBox(-0.5f, 0.0f, -0.5f, 1, 2, 1);
        this.beak2.setPos(0.0f, -1.75f, -2.45f);
        this.head.addChild(this.beak2);
        (this.feather = new ModelPart(this, 2, 18)).addBox(0.0f, -4.0f, -2.0f, 0, 5, 4);
        this.feather.setPos(0.0f, -2.15f, 0.15f);
        this.head.addChild(this.feather);
        (this.legLeft = new ModelPart(this, 14, 18)).addBox(-0.5f, 0.0f, -0.5f, 1, 2, 1);
        this.legLeft.setPos(1.0f, 22.0f, -1.05f);
        (this.legRight = new ModelPart(this, 14, 18)).addBox(-0.5f, 0.0f, -0.5f, 1, 2, 1);
        this.legRight.setPos(-1.0f, 22.0f, -1.05f);
    }
    
    @Override
    public void setupAnim(final Parrot arm, final float float2, final float float3, final float float4, final float float5, final float float6, final float float7) {
        this.render(float7);
    }
    
    @Override
    public void setupAnim(final Parrot arm, final float float2, final float float3, final float float4, final float float5, final float float6, final float float7) {
        this.setupAnim(getState(arm), arm.tickCount, float2, float3, float4, float5, float6);
    }
    
    @Override
    public void prepareMobModel(final Parrot arm, final float float2, final float float3, final float float4) {
        this.prepare(getState(arm));
    }
    
    public void renderOnShoulder(final float float1, final float float2, final float float3, final float float4, final float float5, final int integer) {
        this.prepare(State.ON_SHOULDER);
        this.setupAnim(State.ON_SHOULDER, integer, float1, float2, 0.0f, float3, float4);
        this.render(float5);
    }
    
    private void render(final float float1) {
        this.body.render(float1);
        this.wingLeft.render(float1);
        this.wingRight.render(float1);
        this.tail.render(float1);
        this.head.render(float1);
        this.legLeft.render(float1);
        this.legRight.render(float1);
    }
    ...
    private static State getState(final Parrot arm) {
        if (arm.isPartyParrot()) {
            return State.PARTY;
        }
        if (arm.isSitting()) {
