...
public class BlockElement {
    ...
    public BlockElement(final Vector3f b1, final Vector3f b2, final Map<Direction, BlockElementFace> map, @Nullable final BlockElementRotation doc, final boolean boolean5) {
        this.from = b1;
        this.to = b2;
        this.faces = map;
        this.rotation = doc;
    ...
    private float[] uvsByFace(final Direction fb) {
        switch (fb) {
            case DOWN: {
    ...
    public static class Deserializer implements JsonDeserializer<BlockElement> {
        ...
        public BlockElement deserialize(final JsonElement jsonElement, final Type type, final JsonDeserializationContext jsonDeserializationContext) throws JsonParseException {
            ...
            final Vector3f b6 = this.getFrom(jsonObject5);
            final Vector3f b7 = this.getTo(jsonObject5);
            final BlockElementRotation doc8 = this.getRotation(jsonObject5);
        ...
        private BlockElementRotation getRotation(final JsonObject jsonObject) {
            ...
            if (jsonObject.has("rotation")) {
                ...
                doc3 = new BlockElementRotation(b5, a6, float7, boolean8);
            }
            return doc3;
        }
        ...
        private Map<Direction, BlockElementFace> filterNullFromFaces(final JsonDeserializationContext jsonDeserializationContext, final JsonObject jsonObject) {
            ...
            for (final Map.Entry<String, JsonElement> entry7 : jsonObject2.entrySet()) {
                final Direction fb8 = this.getFacing((String)entry7.getKey());
                map4.put(fb8, jsonDeserializationContext.deserialize((JsonElement)entry7.getValue(), (Type)BlockElementFace.class));
            }
            return map4;
        }
        
        private Direction getFacing(final String string) {
            final Direction fb3 = Direction.byName(string);
            if (fb3 == null) {
        ...
        private Vector3f getTo(final JsonObject jsonObject) {
            final Vector3f b3 = this.getVector3f(jsonObject, "to");
            if (b3.x() < -16.0f || b3.y() < -16.0f || b3.z() < -16.0f || b3.x() > 32.0f || b3.y() > 32.0f || b3.z() > 32.0f) {
                throw new JsonParseException(new StringBuilder().append("'to' specifier exceeds the allowed boundaries: ").append(b3).toString());
            }
            return b3;
        }
        
        private Vector3f getFrom(final JsonObject jsonObject) {
            final Vector3f b3 = this.getVector3f(jsonObject, "from");
            if (b3.x() < -16.0f || b3.y() < -16.0f || b3.z() < -16.0f || b3.x() > 32.0f || b3.y() > 32.0f || b3.z() > 32.0f) {
                throw new JsonParseException(new StringBuilder().append("'from' specifier exceeds the allowed boundaries: ").append(b3).toString());
            }
            return b3;
        }
