...
public class ItemModelGenerator {
    ...
    public BlockModel generateBlockModel(final Function<ResourceLocation, TextureAtlasSprite> function, final BlockModel doe) {
        final Map<String, String> map4 = (Map<String, String>)Maps.newHashMap();
        ...
        for (int integer6 = 0; integer6 < ItemModelGenerator.LAYERS.size(); ++integer6) {
            ...
            if (!doe.hasTexture(string7)) {
                break;
            }
            final String string8 = doe.getTexture(string7);
            map4.put(string7, string8);
            final TextureAtlasSprite dxb9 = (TextureAtlasSprite)function.apply(new ResourceLocation(string8));
            list5.addAll((Collection)this.processFrames(integer6, string7, dxb9));
        }
        map4.put("particle", (doe.hasTexture("particle") ? doe.getTexture("particle") : map4.get("layer0")));
        final BlockModel doe2 = new BlockModel(null, list5, map4, false, false, doe.getTransforms(), doe.getOverrides());
        doe2.name = doe.name;
        return doe2;
    }
    
    private List<BlockElement> processFrames(final int integer, final String string, final TextureAtlasSprite dxb) {
        ...
        list6.addAll((Collection)this.createSideElements(dxb, string, integer));
        return list6;
    }
    
    private List<BlockElement> createSideElements(final TextureAtlasSprite dxb, final String string, final int integer) {
        final float float5 = (float)dxb.getWidth();
        final float float6 = (float)dxb.getHeight();
        final List<BlockElement> list7 = (List<BlockElement>)Lists.newArrayList();
        for (final Span a9 : this.getSpans(dxb)) {
            float float7 = 0.0f;
            float float8 = 0.0f;
            float float9 = 0.0f;
            float float10 = 0.0f;
    ...
    private List<Span> getSpans(final TextureAtlasSprite dxb) {
        final int integer3 = dxb.getWidth();
        final int integer4 = dxb.getHeight();
        final List<Span> list5 = (List<Span>)Lists.newArrayList();
        for (int integer5 = 0; integer5 < dxb.getFrameCount(); ++integer5) {
            for (int integer6 = 0; integer6 < integer4; ++integer6) {
                for (int integer7 = 0; integer7 < integer3; ++integer7) {
                    final boolean boolean9 = !this.isTransparent(dxb, integer5, integer7, integer6, integer3, integer4);
                    this.checkTransition(SpanFacing.UP, list5, dxb, integer5, integer7, integer6, integer3, integer4, boolean9);
                    this.checkTransition(SpanFacing.DOWN, list5, dxb, integer5, integer7, integer6, integer3, integer4, boolean9);
                    this.checkTransition(SpanFacing.LEFT, list5, dxb, integer5, integer7, integer6, integer3, integer4, boolean9);
                    this.checkTransition(SpanFacing.RIGHT, list5, dxb, integer5, integer7, integer6, integer3, integer4, boolean9);
                }
            }
        }
        return list5;
    ...
    private void checkTransition(final SpanFacing b, final List<Span> list, final TextureAtlasSprite dxb, final int integer4, final int integer5, final int integer6, final int integer7, final int integer8, final boolean boolean9) {
        final boolean boolean10 = this.isTransparent(dxb, integer4, integer5 + b.getXOffset(), integer6 + b.getYOffset(), integer7, integer8) && boolean9;
        if (boolean10) {
    ...
    private boolean isTransparent(final TextureAtlasSprite dxb, final int integer2, final int integer3, final int integer4, final int integer5, final int integer6) {
        return integer3 < 0 || integer4 < 0 || integer3 >= integer5 || integer4 >= integer6 || dxb.isTransparent(integer2, integer3, integer4);
    }
    ...
    enum SpanFacing {
        ...
        private SpanFacing(final Direction fb, final int integer4, final int integer5) {
            this.direction = fb;
