...
public class VisGraph {
    ...
    public void setOpaque(final BlockPos ew) {
        this.bitSet.set(getIndex(ew), true);
        --this.empty;
    }
    
    private static int getIndex(final BlockPos ew) {
        return getIndex(ew.getX() & 0xF, ew.getY() & 0xF, ew.getZ() & 0xF);
    }
    ...
    public VisibilitySet resolve() {
        final VisibilitySet dqc2 = new VisibilitySet();
        if (4096 - this.empty < 256) {
            ...
            for (final int integer6 : VisGraph.INDEX_OF_EDGES) {
                if (!this.bitSet.get(integer6)) {
                    dqc2.add(this.floodFill(integer6));
                }
            }
        }
        return dqc2;
    ...
    public Set<Direction> floodFill(final BlockPos ew) {
        return this.floodFill(getIndex(ew));
    }
    ...
    private Set<Direction> floodFill(final int integer) {
        ...
        while (!intPriorityQueue4.isEmpty()) {
            ...
            for (final Direction fb9 : VisGraph.DIRECTIONS) {
                final int integer3 = this.getNeighborIndexAtFace(integer2, fb9);
    ...
    private int getNeighborIndexAtFace(final int integer, final Direction fb) {
        switch (fb) {
            case DOWN: {
