...
public class RabbitRenderer extends MobRenderer<Rabbit, RabbitModel<Rabbit>> {
    ...
    public RabbitRenderer(final EntityRenderDispatcher dsa) {
        super(dsa, new RabbitModel(), 0.3f);
    }
    
    protected ResourceLocation getTextureLocation(final Rabbit arq) {
        final String string3 = ChatFormatting.stripFormatting(arq.getName().getString());
        if (string3 != null && "Toast".equals(string3)) {
            return RabbitRenderer.RABBIT_TOAST_LOCATION;
        }
        switch (arq.getRabbitType()) {
            default: {
