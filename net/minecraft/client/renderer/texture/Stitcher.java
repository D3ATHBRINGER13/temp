...
public class Stitcher {
    ...
    public void registerSprite(final TextureAtlasSprite dxb) {
        final Holder a3 = new Holder(dxb, this.mipLevel);
        this.texturesToBeStitched.add(a3);
    }
    ...
    public void stitch() {
        ...
        for (final Holder a4 : list2) {
            if (!this.addToStorage(a4)) {
                throw new StitcherException(a4.sprite, (Collection<TextureAtlasSprite>)list2.stream().map(a -> a.sprite).collect(ImmutableList.toImmutableList()));
            }
        }
    ...
    public List<TextureAtlasSprite> gatherSprites() {
        ...
        for (final Region b4 : this.storage) {
            b4.walk((Consumer<Region>)(b -> {
                ...
                final TextureAtlasSprite dxb5 = a4.sprite;
                dxb5.init(this.storageX, this.storageY, b.getX(), b.getY());
                list2.add(dxb5);
            }));
        }
        return list2;
    ...
    static {
        HOLDER_COMPARATOR = Comparator.comparing(a -> -a.height).thenComparing(a -> -a.width).thenComparing(a -> a.sprite.getName());
    }
    ...
    static class Holder {
        ...
        public Holder(final TextureAtlasSprite dxb, final int integer) {
            this.sprite = dxb;
            this.width = smallestFittingMinTexel(dxb.getWidth(), integer);
            this.height = smallestFittingMinTexel(dxb.getHeight(), integer);
        }
