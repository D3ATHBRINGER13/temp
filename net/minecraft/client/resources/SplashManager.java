...
public class SplashManager extends SimplePreparableReloadListener<List<String>> {
    ...
    public SplashManager(final User cyn) {
        this.splashes = (List<String>)Lists.newArrayList();
        this.user = cyn;
    }
    
    @Override
    protected List<String> prepare(final ResourceManager xi, final ProfilerFiller agn) {
        try (final Resource xh4 = Minecraft.getInstance().getResourceManager().getResource(SplashManager.SPLASHES_LOCATION);
             final BufferedReader bufferedReader6 = new BufferedReader((Reader)new InputStreamReader(xh4.getInputStream(), StandardCharsets.UTF_8))) {
    ...
    @Override
    protected void apply(final List<String> list, final ResourceManager xi, final ProfilerFiller agn) {
        this.splashes.clear();
