...
public class ReloadableIdSearchTree<T> implements MutableSearchTree<T> {
    ...
    protected void index(final T object) {
        ((Stream)this.idGetter.apply(object)).forEach(qv -> {
            this.namespaceTree.add(object, qv.getNamespace().toLowerCase(Locale.ROOT));
            this.pathTree.add(object, qv.getPath().toLowerCase(Locale.ROOT));
        });
    }
