...
public class EntityAnchorArgument implements ArgumentType<Anchor> {
    ...
    public enum Anchor {
        FEET("feet", (BiFunction<Vec3, Entity, Vec3>)((csi, aio) -> csi)), 
        EYES("eyes", (BiFunction<Vec3, Entity, Vec3>)((csi, aio) -> new Vec3(csi.x, csi.y + aio.getEyeHeight(), csi.z)));
        ...
        public Vec3 apply(final Entity aio) {
            return (Vec3)this.transform.apply(new Vec3(aio.x, aio.y, aio.z), aio);
        }
        
        public Vec3 apply(final CommandSourceStack cd) {
            final Entity aio3 = cd.getEntity();
            if (aio3 == null) {
                return cd.getPosition();
            }
            return (Vec3)this.transform.apply(cd.getPosition(), aio3);
        }
