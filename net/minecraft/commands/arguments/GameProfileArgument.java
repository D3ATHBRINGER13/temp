...
public class GameProfileArgument implements ArgumentType<Result> {
    ...
    public Result parse(final StringReader stringReader) throws CommandSyntaxException {
        if (!stringReader.canRead() || stringReader.peek() != '@') {
            ...
            return cd -> {
                gameProfile3 = cd.getServer().getProfileCache().get(string4);
    ...
    public <S> CompletableFuture<Suggestions> listSuggestions(final CommandContext<S> commandContext, final SuggestionsBuilder suggestionsBuilder) {
        if (commandContext.getSource() instanceof SharedSuggestionProvider) {
            ...
            final EntitySelectorParser ed5 = new EntitySelectorParser(stringReader4);
            try {
                ed5.parse();
            }
            catch (CommandSyntaxException ex) {}
            return ed5.fillSuggestions(suggestionsBuilder, (Consumer<SuggestionsBuilder>)(suggestionsBuilder -> SharedSuggestionProvider.suggest((Iterable<String>)((SharedSuggestionProvider)commandContext.getSource()).getOnlinePlayerNames(), suggestionsBuilder)));
        }
    ...
    static {
        ...
        ERROR_UNKNOWN_PLAYER = new SimpleCommandExceptionType((Message)new TranslatableComponent("argument.player.unknown", new Object[0]));
    }
    ...
    public static class SelectorResult implements Result {
        ...
        public Collection<GameProfile> getNames(final CommandSourceStack cd) throws CommandSyntaxException {
            final List<ServerPlayer> list3 = this.selector.findPlayers(cd);
            if (list3.isEmpty()) {
                throw EntityArgument.NO_PLAYERS_FOUND.create();
            }
            final List<GameProfile> list4 = (List<GameProfile>)Lists.newArrayList();
            for (final ServerPlayer vl6 : list3) {
                list4.add(vl6.getGameProfile());
            }
    ...
    public interface Result {
        Collection<GameProfile> getNames(final CommandSourceStack cd) throws CommandSyntaxException;
    }
}
