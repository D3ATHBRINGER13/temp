...
public class MessageArgument implements ArgumentType<Message> {
    ...
    public static class Message {
        ...
        public Component toComponent(final CommandSourceStack cd, final boolean boolean2) throws CommandSyntaxException {
            if (this.parts.length == 0 || !boolean2) {
                return new TextComponent(this.text);
            }
            final Component jo4 = new TextComponent(this.text.substring(0, this.parts[0].getStart()));
            int integer5 = this.parts[0].getStart();
            for (final Part b9 : this.parts) {
                final Component jo5 = b9.toComponent(cd);
                if (integer5 < b9.getStart()) {
                    jo4.append(this.text.substring(integer5, b9.getStart()));
                }
                if (jo5 != null) {
                    jo4.append(jo5);
                }
            ...
            if (integer5 < this.text.length()) {
                jo4.append(this.text.substring(integer5, this.text.length()));
            }
            return jo4;
        }
        ...
        public static Message parseText(final StringReader stringReader, final boolean boolean2) throws CommandSyntaxException {
            ...
            while (stringReader.canRead()) {
                if (stringReader.peek() == '@') {
                    ...
                    EntitySelector ec7;
                    try {
                        final EntitySelectorParser ed8 = new EntitySelectorParser(stringReader);
                        ec7 = ed8.parse();
                    }
                    ...
                    list4.add(new Part(integer6 - integer5, stringReader.getCursor() - integer5, ec7));
                }
                else {
                    stringReader.skip();
                }
    ...
    public static class Part {
        ...
        public Part(final int integer1, final int integer2, final EntitySelector ec) {
            this.start = integer1;
            this.end = integer2;
            this.selector = ec;
        }
        ...
        @Nullable
        public Component toComponent(final CommandSourceStack cd) throws CommandSyntaxException {
