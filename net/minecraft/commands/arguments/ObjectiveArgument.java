...
public class ObjectiveArgument implements ArgumentType<String> {
    ...
    public static Objective getObjective(final CommandContext<CommandSourceStack> commandContext, final String string) throws CommandSyntaxException {
        ...
        final Scoreboard cti4 = ((CommandSourceStack)commandContext.getSource()).getServer().getScoreboard();
        final Objective ctf5 = cti4.getObjective(string2);
        if (ctf5 == null) {
    ...
    public static Objective getWritableObjective(final CommandContext<CommandSourceStack> commandContext, final String string) throws CommandSyntaxException {
        final Objective ctf3 = getObjective(commandContext, string);
        if (ctf3.getCriteria().isReadOnly()) {
            throw ObjectiveArgument.ERROR_OBJECTIVE_READ_ONLY.create(ctf3.getName());
        }
        return ctf3;
    }
    ...
    public <S> CompletableFuture<Suggestions> listSuggestions(final CommandContext<S> commandContext, final SuggestionsBuilder suggestionsBuilder) {
        ...
        if (commandContext.getSource() instanceof SharedSuggestionProvider) {
            final SharedSuggestionProvider cf4 = (SharedSuggestionProvider)commandContext.getSource();
            return cf4.customSuggestion((CommandContext<SharedSuggestionProvider>)commandContext, suggestionsBuilder);
        }
