...
public class ParticleArgument implements ArgumentType<ParticleOptions> {
    ...
    public static ParticleOptions readParticle(final StringReader stringReader) throws CommandSyntaxException {
        final ResourceLocation qv2 = ResourceLocation.read(stringReader);
        final ParticleType<?> gg3 = Registry.PARTICLE_TYPE.getOptional(qv2).orElseThrow(() -> ParticleArgument.ERROR_UNKNOWN_PARTICLE.create(qv2));
        return ParticleArgument.<ParticleOptions>readParticle(stringReader, gg3);
    }
    
    private static <T extends ParticleOptions> T readParticle(final StringReader stringReader, final ParticleType<T> gg) throws CommandSyntaxException {
        return gg.getDeserializer().fromCommand(gg, stringReader);
    }
