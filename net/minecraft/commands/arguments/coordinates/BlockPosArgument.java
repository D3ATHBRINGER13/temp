...
public class BlockPosArgument implements ArgumentType<Coordinates> {
    ...
    public static BlockPos getLoadedBlockPos(final CommandContext<CommandSourceStack> commandContext, final String string) throws CommandSyntaxException {
        final BlockPos ew3 = ((Coordinates)commandContext.getArgument(string, (Class)Coordinates.class)).getBlockPos((CommandSourceStack)commandContext.getSource());
        if (!((CommandSourceStack)commandContext.getSource()).getLevel().hasChunkAt(ew3)) {
            throw BlockPosArgument.ERROR_NOT_LOADED.create();
        }
        ((CommandSourceStack)commandContext.getSource()).getLevel();
        if (!Level.isInWorldBounds(ew3)) {
    ...
    static {
        ...
        ERROR_NOT_LOADED = new SimpleCommandExceptionType((Message)new TranslatableComponent("argument.pos.unloaded", new Object[0]));
        ERROR_OUT_OF_WORLD = new SimpleCommandExceptionType((Message)new TranslatableComponent("argument.pos.outofworld", new Object[0]));
    }
}
