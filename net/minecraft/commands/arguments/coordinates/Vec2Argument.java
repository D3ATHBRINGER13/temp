...
public class Vec2Argument implements ArgumentType<Coordinates> {
    ...
    public static Vec2 getVec2(final CommandContext<CommandSourceStack> commandContext, final String string) throws CommandSyntaxException {
        final Vec3 csi3 = ((Coordinates)commandContext.getArgument(string, (Class)Coordinates.class)).getPosition((CommandSourceStack)commandContext.getSource());
        return new Vec2((float)csi3.x, (float)csi3.z);
    }
    ...
    public Coordinates parse(final StringReader stringReader) throws CommandSyntaxException {
        ...
        final WorldCoordinate ds4 = WorldCoordinate.parseDouble(stringReader, this.centerCorrect);
        if (!stringReader.canRead() || stringReader.peek() != ' ') {
            stringReader.setCursor(integer3);
            throw Vec2Argument.ERROR_NOT_COMPLETE.createWithContext((ImmutableStringReader)stringReader);
        }
        stringReader.skip();
        final WorldCoordinate ds5 = WorldCoordinate.parseDouble(stringReader, this.centerCorrect);
        return new WorldCoordinates(ds4, new WorldCoordinate(true, 0.0), ds5);
    }
    ...
    static {
        ...
        ERROR_NOT_COMPLETE = new SimpleCommandExceptionType((Message)new TranslatableComponent("argument.pos2d.incomplete", new Object[0]));
    }
}
