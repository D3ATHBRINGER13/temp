...
public class ItemParser {
    ...
    public void readItem() throws CommandSyntaxException {
        ...
        final ResourceLocation qv3 = ResourceLocation.read(this.reader);
        this.item = (Item)Registry.ITEM.getOptional(qv3).orElseThrow(() -> {
            this.reader.setCursor(integer2);
            return ItemParser.ERROR_UNKNOWN_ITEM.createWithContext((ImmutableStringReader)this.reader, qv3.toString());
        });
    }
    ...
    public ItemParser parse() throws CommandSyntaxException {
        this.suggestions = (Function<SuggestionsBuilder, CompletableFuture<Suggestions>>)this::suggestItemIdOrTag;
    ...
    private CompletableFuture<Suggestions> suggestTag(final SuggestionsBuilder suggestionsBuilder) {
        return SharedSuggestionProvider.suggestResource((Iterable<ResourceLocation>)ItemTags.getAllTags().getAvailableTags(), suggestionsBuilder.createOffset(this.tagCursor));
    }
    
    private CompletableFuture<Suggestions> suggestItemIdOrTag(final SuggestionsBuilder suggestionsBuilder) {
        if (this.forTesting) {
            SharedSuggestionProvider.suggestResource((Iterable<ResourceLocation>)ItemTags.getAllTags().getAvailableTags(), suggestionsBuilder, String.valueOf('#'));
        }
    ...
    static {
        ERROR_NO_TAGS_ALLOWED = new SimpleCommandExceptionType((Message)new TranslatableComponent("argument.item.tag.disallowed", new Object[0]));
