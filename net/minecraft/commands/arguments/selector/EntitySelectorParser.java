...
public class EntitySelectorParser {
    ...
    public EntitySelectorParser(final StringReader stringReader, final boolean boolean2) {
        ...
        this.predicate = (Predicate<Entity>)(aio -> true);
    ...
    public EntitySelector getSelector() {
        ...
        if (this.deltaX != null || this.deltaY != null || this.deltaZ != null) {
            csc2 = this.createAabb((this.deltaX == null) ? 0.0 : ((double)this.deltaX), (this.deltaY == null) ? 0.0 : ((double)this.deltaY), (this.deltaZ == null) ? 0.0 : ((double)this.deltaZ));
        }
        else if (this.distance.getMax() != null) {
            final float float3 = this.distance.getMax();
            csc2 = new AABB(-float3, -float3, -float3, float3 + 1.0f, float3 + 1.0f, float3 + 1.0f);
        }
        else {
            csc2 = null;
        }
        ...
        if (this.x == null && this.y == null && this.z == null) {
            function3 = (Function<Vec3, Vec3>)(csi -> csi);
        }
        else {
            function3 = (Function<Vec3, Vec3>)(csi -> new Vec3((this.x == null) ? csi.x : this.x, (this.y == null) ? csi.y : this.y, (this.z == null) ? csi.z : this.z));
        }
        return new EntitySelector(this.maxResults, this.includesEntities, this.worldLimited, this.predicate, this.distance, function3, csc2, this.order, this.currentEntity, this.playerName, this.entityUUID, this.type, this.usesSelectors);
    }
    ...
    private void finalizePredicates() {
        if (this.rotX != WrappedMinMaxBounds.ANY) {
            this.predicate = (Predicate<Entity>)this.predicate.and((Predicate)this.createRotationPredicate(this.rotX, (ToDoubleFunction<Entity>)(aio -> aio.xRot)));
        }
        if (this.rotY != WrappedMinMaxBounds.ANY) {
            this.predicate = (Predicate<Entity>)this.predicate.and((Predicate)this.createRotationPredicate(this.rotY, (ToDoubleFunction<Entity>)(aio -> aio.yRot)));
        }
        if (!this.level.isAny()) {
            this.predicate = (Predicate<Entity>)this.predicate.and(aio -> aio instanceof ServerPlayer && this.level.matches(((ServerPlayer)aio).experienceLevel));
        }
    }
    
    private Predicate<Entity> createRotationPredicate(final WrappedMinMaxBounds bw, final ToDoubleFunction<Entity> toDoubleFunction) {
        final double double4 = Mth.wrapDegrees((bw.getMin() == null) ? 0.0f : ((float)bw.getMin()));
        final double double5 = Mth.wrapDegrees((bw.getMax() == null) ? 359.0f : ((float)bw.getMax()));
        return (Predicate<Entity>)(aio -> {
            final double double4 = Mth.wrapDegrees(toDoubleFunction.applyAsDouble(aio));
            if (double4 > double5) {
    ...
    public void setRotX(final WrappedMinMaxBounds bw) {
        this.rotX = bw;
    }
    ...
    public void setRotY(final WrappedMinMaxBounds bw) {
        this.rotY = bw;
    }
    ...
    public void limitToType(final EntityType<?> ais) {
        this.type = ais;
    }
    ...
    static {
        ERROR_INVALID_NAME_OR_UUID = new SimpleCommandExceptionType((Message)new TranslatableComponent("argument.entity.invalid", new Object[0]));
        ...
        ERROR_MISSING_SELECTOR_TYPE = new SimpleCommandExceptionType((Message)new TranslatableComponent("argument.entity.selector.missing", new Object[0]));
        ERROR_EXPECTED_END_OF_OPTIONS = new SimpleCommandExceptionType((Message)new TranslatableComponent("argument.entity.options.unterminated", new Object[0]));
        ...
        ORDER_ARBITRARY = ((csi, list) -> {});
        ORDER_NEAREST = ((csi, list) -> list.sort((aio2, aio3) -> Doubles.compare(aio2.distanceToSqr(csi), aio3.distanceToSqr(csi))));
        ORDER_FURTHEST = ((csi, list) -> list.sort((aio2, aio3) -> Doubles.compare(aio3.distanceToSqr(csi), aio2.distanceToSqr(csi))));
        ORDER_RANDOM = ((csi, list) -> Collections.shuffle(list));
