...
public class LongArgumentSerializer implements ArgumentSerializer<LongArgumentType> {
    public void serializeToNetwork(final LongArgumentType longArgumentType, final FriendlyByteBuf je) {
        ...
        if (boolean4) {
            je.writeLong(longArgumentType.getMinimum());
        }
        if (boolean5) {
            je.writeLong(longArgumentType.getMaximum());
        }
    }
    
    public LongArgumentType deserializeFromNetwork(final FriendlyByteBuf je) {
        final byte byte3 = je.readByte();
        final long long4 = BrigadierArgumentSerializers.numberHasMin(byte3) ? je.readLong() : Long.MIN_VALUE;
        final long long5 = BrigadierArgumentSerializers.numberHasMax(byte3) ? je.readLong() : Long.MAX_VALUE;
