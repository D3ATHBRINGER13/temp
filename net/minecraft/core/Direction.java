...
public enum Direction implements StringRepresentable {
    ...
    private Direction(final int integer3, final int integer4, final int integer5, final String string6, final AxisDirection b, final Axis a, final Vec3i fs) {
    ...
    public static Direction[] orderedByNearest(final Entity aio) {
        final float float2 = aio.getViewXRot(1.0f) * 0.017453292f;
        final float float3 = -aio.getViewYRot(1.0f) * 0.017453292f;
        ...
        final Direction fb16 = boolean8 ? Direction.EAST : Direction.WEST;
        final Direction fb17 = boolean9 ? Direction.UP : Direction.DOWN;
        final Direction fb18 = boolean10 ? Direction.SOUTH : Direction.NORTH;
        if (float8 > float10) {
            if (float9 > float11) {
                return makeDirectionArray(fb17, fb16, fb18);
            }
            if (float12 > float9) {
                return makeDirectionArray(fb16, fb18, fb17);
            }
            return makeDirectionArray(fb16, fb17, fb18);
        }
        else {
            if (float9 > float12) {
                return makeDirectionArray(fb17, fb18, fb16);
            }
            if (float11 > float9) {
                return makeDirectionArray(fb18, fb16, fb17);
            }
            return makeDirectionArray(fb18, fb17, fb16);
        }
    }
    
    private static Direction[] makeDirectionArray(final Direction fb1, final Direction fb2, final Direction fb3) {
        return new Direction[] { fb1, fb2, fb3, fb3.getOpposite(), fb2.getOpposite(), fb1.getOpposite() };
    }
    ...
    public Direction getClockWise(final Axis a) {
        switch (a) {
            case X: {
            ...
            default: {
                throw new IllegalStateException(new StringBuilder().append("Unable to get CW facing for axis ").append(a).toString());
            }
        }
    }
    ...
    private Direction getClockWiseX() {
        switch (this) {
            ...
            default: {
                throw new IllegalStateException(new StringBuilder().append("Unable to get X-rotated facing of ").append(this).toString());
            }
        }
    }
    ...
    private Direction getClockWiseZ() {
        switch (this) {
            ...
            default: {
                throw new IllegalStateException(new StringBuilder().append("Unable to get Z-rotated facing of ").append(this).toString());
            }
        }
    }
    ...
    public static Direction getRandomFace(final Random random) {
    ...
    public static Direction getNearest(final float float1, final float float2, final float float3) {
        Direction fb4 = Direction.NORTH;
        float float4 = Float.MIN_VALUE;
        for (final Direction fb5 : Direction.VALUES) {
            final float float5 = float1 * fb5.normal.getX() + float2 * fb5.normal.getY() + float3 * fb5.normal.getZ();
            if (float5 > float4) {
                float4 = float5;
                fb4 = fb5;
            }
    ...
    public static Direction get(final AxisDirection b, final Axis a) {
        for (final Direction fb6 : values()) {
            if (fb6.getAxisDirection() == b && fb6.getAxis() == a) {
                return fb6;
            }
        }
    ...
    static {
        VALUES = values();
        BY_NAME = (Map)Arrays.stream((Object[])Direction.VALUES).collect(Collectors.toMap(Direction::getName, fb -> fb));
        BY_3D_DATA = (Direction[])Arrays.stream((Object[])Direction.VALUES).sorted(Comparator.comparingInt(fb -> fb.data3d)).toArray(Direction[]::new);
        BY_2D_DATA = (Direction[])Arrays.stream((Object[])Direction.VALUES).filter(fb -> fb.getAxis().isHorizontal()).sorted(Comparator.comparingInt(fb -> fb.data2d)).toArray(Direction[]::new);
        BY_NORMAL = (Long2ObjectMap)Arrays.stream((Object[])Direction.VALUES).collect(Collectors.toMap(fb -> new BlockPos(fb.getNormal()).asLong(), fb -> fb, (fb1, fb2) -> {
    ...
    public enum Axis implements StringRepresentable, Predicate<Direction> {
        ...
        public static Axis getRandomAxis(final Random random) {
            return values()[random.nextInt(values().length)];
        }
        
        public boolean test(@Nullable final Direction fb) {
            return fb != null && fb.getAxis() == this;
        }
        ...
        static {
            BY_NAME = (Map)Arrays.stream((Object[])values()).collect(Collectors.toMap(Axis::getName, a -> a));
        }
    }
    ...
    public enum Plane implements Iterable<Direction>, Predicate<Direction> {
        ...
        public boolean test(@Nullable final Direction fb) {
            return fb != null && fb.getAxis().getPlane() == this;
        }
