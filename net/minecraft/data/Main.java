...
public class Main {
    public static void main(final String[] arr) throws IOException {
        ...
        final DataGenerator gk20 = createStandardGenerator(path13, (Collection<Path>)optionSet12.valuesOf((OptionSpec)optionSpec11).stream().map(string -> Paths.get(string, new String[0])).collect(Collectors.toList()), boolean15, boolean16, boolean17, boolean18, boolean19);
        gk20.run();
    }
    ...
    public static DataGenerator createStandardGenerator(final Path path, final Collection<Path> collection, final boolean boolean3, final boolean boolean4, final boolean boolean5, final boolean boolean6, final boolean boolean7) {
        final DataGenerator gk8 = new DataGenerator(path, collection);
        if (boolean3 || boolean4) {
            gk8.addProvider(new SnbtToNbt(gk8).addFilter(new StructureUpdater()));
        }
        if (boolean4) {
            gk8.addProvider(new FluidTagsProvider(gk8));
            gk8.addProvider(new BlockTagsProvider(gk8));
            gk8.addProvider(new ItemTagsProvider(gk8));
            gk8.addProvider(new EntityTypeTagsProvider(gk8));
            gk8.addProvider(new RecipeProvider(gk8));
            gk8.addProvider(new AdvancementProvider(gk8));
            gk8.addProvider(new LootTableProvider(gk8));
        }
        if (boolean5) {
            gk8.addProvider(new NbtToSnbt(gk8));
        }
        if (boolean6) {
            gk8.addProvider(new BlockListReport(gk8));
            gk8.addProvider(new RegistryDumpReport(gk8));
            gk8.addProvider(new CommandsReport(gk8));
        }
        return gk8;
    }
}
