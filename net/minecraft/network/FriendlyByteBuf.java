...
public class FriendlyByteBuf extends ByteBuf {
    ...
    public FriendlyByteBuf writeBlockPos(final BlockPos ew) {
        this.writeLong(ew.asLong());
        return this;
    }
    ...
    public FriendlyByteBuf writeComponent(final Component jo) {
        return this.writeUtf(Component.Serializer.toJson(jo), 262144);
    }
    ...
    public FriendlyByteBuf writeNbt(@Nullable final CompoundTag id) {
        if (id == null) {
            this.writeByte(0);
        }
        else {
            try {
                NbtIo.write(id, (DataOutput)new ByteBufOutputStream((ByteBuf)this));
            }
    ...
    public CompoundTag readNbt() {
        final int integer2 = this.readerIndex();
        ...
        try {
            return NbtIo.read((DataInput)new ByteBufInputStream((ByteBuf)this), new NbtAccounter(2097152L));
        }
        catch (IOException iOException4) {
            throw new EncoderException((Throwable)iOException4);
        }
    }
    
    public FriendlyByteBuf writeItem(final ItemStack bcj) {
        if (bcj.isEmpty()) {
            ...
            this.writeVarInt(Item.getId(bce3));
            this.writeByte(bcj.getCount());
            CompoundTag id4 = null;
            if (bce3.canBeDepleted() || bce3.shouldOverrideMultiplayerNbt()) {
                id4 = bcj.getTag();
            }
            this.writeNbt(id4);
        }
    ...
    public ItemStack readItem() {
        ...
        final ItemStack bcj4 = new ItemStack(Item.byId(integer2), integer3);
        bcj4.setTag(this.readNbt());
        return bcj4;
    }
    ...
    public FriendlyByteBuf writeResourceLocation(final ResourceLocation qv) {
        this.writeUtf(qv.toString());
        return this;
    }
    ...
    public BlockHitResult readBlockHitResult() {
        final BlockPos ew2 = this.readBlockPos();
        final Direction fb3 = this.<Direction>readEnum(Direction.class);
        ...
        return new BlockHitResult(new Vec3(ew2.getX() + float4, ew2.getY() + float5, ew2.getZ() + float6), fb3, ew2, boolean7);
    }
    
    public void writeBlockHitResult(final BlockHitResult csd) {
        final BlockPos ew3 = csd.getBlockPos();
        this.writeBlockPos(ew3);
        this.writeEnum(csd.getDirection());
        final Vec3 csi4 = csd.getLocation();
        this.writeFloat((float)(csi4.x - ew3.getX()));
        this.writeFloat((float)(csi4.y - ew3.getY()));
        this.writeFloat((float)(csi4.z - ew3.getZ()));
        this.writeBoolean(csd.isInside());
    }
