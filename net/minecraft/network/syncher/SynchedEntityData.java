...
public class SynchedEntityData {
    ...
    public SynchedEntityData(final Entity aio) {
    ...
    public static <T> EntityDataAccessor<T> defineId(final Class<? extends Entity> class1, final EntityDataSerializer<T> ql) {
        ...
        return ql.createAccessor(integer3);
    }
    
    public <T> void define(final EntityDataAccessor<T> qk, final T object) {
        final int integer4 = qk.getId();
        if (integer4 > 254) {
        ...
        if (EntityDataSerializers.getSerializedId(qk.getSerializer()) < 0) {
            throw new IllegalArgumentException(new StringBuilder().append("Unregistered serializer ").append(qk.getSerializer()).append(" for ").append(integer4).append("!").toString());
        }
        this.createDataItem((EntityDataAccessor<Object>)qk, object);
    }
    
    private <T> void createDataItem(final EntityDataAccessor<T> qk, final T object) {
        final DataItem<T> a4 = new DataItem<T>(qk, object);
        this.lock.writeLock().lock();
        this.itemsById.put(qk.getId(), a4);
        this.isEmpty = false;
        this.lock.writeLock().unlock();
    }
    
    private <T> DataItem<T> getItem(final EntityDataAccessor<T> qk) {
        this.lock.readLock().lock();
        DataItem<T> a3;
        try {
            a3 = (DataItem<T>)this.itemsById.get(qk.getId());
        }
        catch (Throwable throwable4) {
            final CrashReport d5 = CrashReport.forThrowable(throwable4, "Getting synched entity data");
            final CrashReportCategory e6 = d5.addCategory("Synched entity data");
            e6.setDetail("Data ID", qk);
    ...
    public <T> T get(final EntityDataAccessor<T> qk) {
        return this.<T>getItem(qk).getValue();
    }
    
    public <T> void set(final EntityDataAccessor<T> qk, final T object) {
        final DataItem<T> a4 = this.<T>getItem(qk);
        if (ObjectUtils.notEqual(object, a4.getValue())) {
            a4.setValue(object);
            this.entity.onSyncedDataUpdated(qk);
            a4.setDirty(true);
            this.isDirty = true;
        }
    }
    ...
    public static void pack(final List<DataItem<?>> list, final FriendlyByteBuf je) throws IOException {
        if (list != null) {
            for (int integer3 = 0, integer4 = list.size(); integer3 < integer4; ++integer3) {
                SynchedEntityData.writeDataItem(je, (DataItem<Object>)list.get(integer3));
            }
        }
        je.writeByte(255);
    }
    ...
    public void packAll(final FriendlyByteBuf je) throws IOException {
        this.lock.readLock().lock();
        for (final DataItem<?> a4 : this.itemsById.values()) {
            SynchedEntityData.writeDataItem(je, a4);
        }
        this.lock.readLock().unlock();
        je.writeByte(255);
    }
    ...
    private static <T> void writeDataItem(final FriendlyByteBuf je, final DataItem<T> a) throws IOException {
        final EntityDataAccessor<T> qk3 = a.getAccessor();
        final int integer4 = EntityDataSerializers.getSerializedId(qk3.getSerializer());
        if (integer4 < 0) {
            throw new EncoderException(new StringBuilder().append("Unknown serializer type ").append(qk3.getSerializer()).toString());
        }
        je.writeByte(qk3.getId());
        je.writeVarInt(integer4);
        qk3.getSerializer().write(je, a.getValue());
    }
    
    @Nullable
    public static List<DataItem<?>> unpack(final FriendlyByteBuf je) throws IOException {
        List<DataItem<?>> list2 = null;
        int integer3;
        while ((integer3 = je.readUnsignedByte()) != 255) {
            if (list2 == null) {
                list2 = (List<DataItem<?>>)Lists.newArrayList();
            }
            final int integer4 = je.readVarInt();
            final EntityDataSerializer<?> ql5 = EntityDataSerializers.getSerializer(integer4);
            if (ql5 == null) {
                throw new DecoderException(new StringBuilder().append("Unknown serializer type ").append(integer4).toString());
            }
            list2.add(SynchedEntityData.genericHelper(je, integer3, ql5));
        }
        return list2;
    }
    
    private static <T> DataItem<T> genericHelper(final FriendlyByteBuf je, final int integer, final EntityDataSerializer<T> ql) {
        return new DataItem<T>(ql.createAccessor(integer), ql.read(je));
    }
    ...
    public static class DataItem<T> {
        ...
        public DataItem(final EntityDataAccessor<T> qk, final T object) {
            this.accessor = qk;
            this.value = object;
            this.dirty = true;
        }
