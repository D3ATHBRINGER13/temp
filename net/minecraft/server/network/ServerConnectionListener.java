    ...
    public void startTcpServerListener(@Nullable final InetAddress inetAddress, final int integer) throws IOException {
        synchronized (this.channels) {
            ...
            if (Epoll.isAvailable() && this.server.isEpollEnabled()) {
                ...
                zv6 = ServerConnectionListener.SERVER_EPOLL_EVENT_GROUP;
                ServerConnectionListener.LOGGER.info("Using epoll channel type");
            }
            else {
                class5 = NioServerSocketChannel.class;
                zv6 = ServerConnectionListener.SERVER_EVENT_GROUP;
            ...
            this.channels.add(((ServerBootstrap)((ServerBootstrap)new ServerBootstrap().channel((Class)class5)).childHandler((ChannelHandler)new ChannelInitializer<Channel>() {
                protected void initChannel(final Channel channel) throws Exception {
                    ...
                    final Connection jc3 = new Connection(PacketFlow.SERVERBOUND);
                    ServerConnectionListener.this.connections.add((Object)jc3);
                    channel.pipeline().addLast("packet_handler", (ChannelHandler)jc3);
                    jc3.setListener(new ServerHandshakePacketListenerImpl(ServerConnectionListener.this.server, jc3));
                }
            }).group((EventLoopGroup)zv6.get()).localAddress(inetAddress, integer)).bind().syncUninterruptibly());
        }
    }
    ...
    public SocketAddress startMemoryChannel() {
        ...
        synchronized (this.channels) {
            channelFuture2 = ((ServerBootstrap)((ServerBootstrap)new ServerBootstrap().channel((Class)LocalServerChannel.class)).childHandler((ChannelHandler)new ChannelInitializer<Channel>() {
                protected void initChannel(final Channel channel) throws Exception {
                    final Connection jc3 = new Connection(PacketFlow.SERVERBOUND);
                    jc3.setListener(new MemoryServerHandshakePacketListenerImpl(ServerConnectionListener.this.server, jc3));
                    ServerConnectionListener.this.connections.add(jc3);
                    channel.pipeline().addLast("packet_handler", (ChannelHandler)jc3);
                }
            ...
            while (iterator3.hasNext()) {
                ...
                if (jc4.isConnected()) {
                    try {
                        ...
                        if (jc4.isMemoryConnection()) {
                            final CrashReport d6 = CrashReport.forThrowable((Throwable)exception5, "Ticking memory connection");
                            final CrashReportCategory e7 = d6.addCategory("Ticking connection");
                            e7.setDetail("Connection", (CrashReportDetail<String>)jc4::toString);
                            throw new ReportedException(d6);
                        }
                        ServerConnectionListener.LOGGER.warn("Failed to handle packet for {}", jc4.getRemoteAddress(), exception5);
                        final Component jo6 = new TextComponent("Internal server error");
                        jc4.send(new ClientboundDisconnectPacket(jo6), (future -> jc4.disconnect(jo6)));
                        jc4.setReadOnly();
                    }
                }
                else {
