...
public class ServerLoginPacketListenerImpl implements ServerLoginPacketListener {
    ...
    public ServerLoginPacketListenerImpl(final MinecraftServer minecraftServer, final Connection jc) {
    ...
    public void tick() {
        if (this.state == State.READY_TO_ACCEPT) {
            ...
            final ServerPlayer vl2 = this.server.getPlayerList().getPlayer(this.gameProfile.getId());
            if (vl2 == null) {
    ...
    public void disconnect(final Component jo) {
        try {
            ServerLoginPacketListenerImpl.LOGGER.info("Disconnecting {}: {}", this.getUserName(), jo.getString());
            this.connection.send(new ClientboundLoginDisconnectPacket(jo));
            this.connection.disconnect(jo);
        }
    ...
    public void handleAcceptedLogin() {
        ...
        final Component jo2 = this.server.getPlayerList().canPlayerLogin(this.connection.getRemoteAddress(), this.gameProfile);
        if (jo2 != null) {
            this.disconnect(jo2);
        }
        else {
            ...
            final ServerPlayer vl3 = this.server.getPlayerList().getPlayer(this.gameProfile.getId());
            if (vl3 != null) {
    ...
    public void onDisconnect(final Component jo) {
        ServerLoginPacketListenerImpl.LOGGER.info("{} lost connection: {}", this.getUserName(), jo.getString());
    }
    ...
    public void handleHello(final ServerboundHelloPacket py) {
        Validate.validState(this.state == State.HELLO, "Unexpected hello packet", new Object[0]);
        this.gameProfile = py.getGameProfile();
    ...
    public void handleKey(final ServerboundKeyPacket pz) {
        ...
        if (!Arrays.equals(this.nonce, pz.getNonce(privateKey3))) {
            throw new IllegalStateException("Invalid nonce!");
        }
        this.secretKey = pz.getSecretKey(privateKey3);
    ...
    public void handleCustomQueryPacket(final ServerboundCustomQueryPacket px) {
