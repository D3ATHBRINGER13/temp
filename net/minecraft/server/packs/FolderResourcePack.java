...
public class FolderResourcePack extends AbstractResourcePack {
    ...
    public static boolean validatePath(final File file, final String string) throws IOException {
        ...
        if (FolderResourcePack.ON_WINDOWS) {
            string2 = FolderResourcePack.BACKSLASH_MATCHER.replaceFrom((CharSequence)string2, '/');
        }
    ...
    public Set<String> getNamespaces(final PackType wm) {
        ...
        final File file4 = new File(this.file, wm.getDirectory());
        final File[] arr5 = file4.listFiles((FileFilter)DirectoryFileFilter.DIRECTORY);
        if (arr5 != null) {
            for (final File file5 : arr5) {
                final String string10 = AbstractResourcePack.getRelativePath(file4, file5);
    ...
    public Collection<ResourceLocation> getResources(final PackType wm, final String string, final int integer, final Predicate<String> predicate) {
        final File file6 = new File(this.file, wm.getDirectory());
        final List<ResourceLocation> list7 = (List<ResourceLocation>)Lists.newArrayList();
        for (final String string2 : this.getNamespaces(wm)) {
            this.listResources(new File(new File(file6, string2), string), integer, string2, list7, string + "/", predicate);
        }
        ...
        if (arr8 != null) {
            for (final File file2 : arr8) {
                if (file2.isDirectory()) {
                    ...
                    catch (ResourceLocationException n13) {
                        FolderResourcePack.LOGGER.error(n13.getMessage());
                    }
                }
            }
        }
