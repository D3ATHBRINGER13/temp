...
public class OldUsersConverter {
    ...
    public static boolean convertUserBanlist(final MinecraftServer minecraftServer) {
        final UserBanList ya2 = new UserBanList(PlayerList.USERBANLIST_FILE);
        if (OldUsersConverter.OLD_USERBANLIST.exists() && OldUsersConverter.OLD_USERBANLIST.isFile()) {
            if (ya2.getFile().exists()) {
                try {
                    ya2.load();
                }
                catch (FileNotFoundException fileNotFoundException3) {
                    OldUsersConverter.LOGGER.warn("Could not load existing file {}", ya2.getFile().getName(), fileNotFoundException3);
                }
            }
            try {
                ...
                final ProfileLookupCallback profileLookupCallback4 = (ProfileLookupCallback)new ProfileLookupCallback() {
                    public void onProfileLookupSucceeded(final GameProfile gameProfile) {
                        ...
                        ((StoredUserList<K, UserBanListEntry>)ya2).add(new UserBanListEntry(gameProfile, date4, string5, date5, string6));
                    }
    ...
    public static boolean convertIpBanlist(final MinecraftServer minecraftServer) {
        final IpBanList xs2 = new IpBanList(PlayerList.IPBANLIST_FILE);
        if (OldUsersConverter.OLD_IPBANLIST.exists() && OldUsersConverter.OLD_IPBANLIST.isFile()) {
            if (xs2.getFile().exists()) {
                try {
                    xs2.load();
                }
                catch (FileNotFoundException fileNotFoundException3) {
                    OldUsersConverter.LOGGER.warn("Could not load existing file {}", xs2.getFile().getName(), fileNotFoundException3);
                }
            }
            try {
                final Map<String, String[]> map3 = (Map<String, String[]>)Maps.newHashMap();
                ...
                for (final String string5 : map3.keySet()) {
                    ...
                    ((StoredUserList<K, IpBanListEntry>)xs2).add(new IpBanListEntry(string5, date7, string6, date8, string7));
                }
                xs2.save();
    ...
    public static boolean convertOpsList(final MinecraftServer minecraftServer) {
        final ServerOpList xw2 = new ServerOpList(PlayerList.OPLIST_FILE);
        if (OldUsersConverter.OLD_OPLIST.exists() && OldUsersConverter.OLD_OPLIST.isFile()) {
            if (xw2.getFile().exists()) {
                try {
                    xw2.load();
                }
                catch (FileNotFoundException fileNotFoundException3) {
                    OldUsersConverter.LOGGER.warn("Could not load existing file {}", xw2.getFile().getName(), fileNotFoundException3);
                }
            }
            try {
                ...
                final ProfileLookupCallback profileLookupCallback4 = (ProfileLookupCallback)new ProfileLookupCallback() {
                    public void onProfileLookupSucceeded(final GameProfile gameProfile) {
                        minecraftServer.getProfileCache().add(gameProfile);
                        ((StoredUserList<K, ServerOpListEntry>)xw2).add(new ServerOpListEntry(gameProfile, minecraftServer.getOperatorUserPermissionLevel(), false));
                    }
    ...
    public static boolean convertWhiteList(final MinecraftServer minecraftServer) {
        final UserWhiteList yc2 = new UserWhiteList(PlayerList.WHITELIST_FILE);
        if (OldUsersConverter.OLD_WHITELIST.exists() && OldUsersConverter.OLD_WHITELIST.isFile()) {
            if (yc2.getFile().exists()) {
                try {
                    yc2.load();
                }
                catch (FileNotFoundException fileNotFoundException3) {
                    OldUsersConverter.LOGGER.warn("Could not load existing file {}", yc2.getFile().getName(), fileNotFoundException3);
                }
            }
            try {
                ...
                final ProfileLookupCallback profileLookupCallback4 = (ProfileLookupCallback)new ProfileLookupCallback() {
                    public void onProfileLookupSucceeded(final GameProfile gameProfile) {
                        minecraftServer.getProfileCache().add(gameProfile);
                        ((StoredUserList<K, UserWhiteListEntry>)yc2).add(new UserWhiteListEntry(gameProfile));
                    }
    ...
    public static String convertMobOwnerIfNecessary(final MinecraftServer minecraftServer, final String string) {
        ...
        if (minecraftServer.isSingleplayer() || !minecraftServer.usesAuthentication()) {
            return Player.createPlayerUUID(new GameProfile((UUID)null, string)).toString();
        }
        ...
        if (!list4.isEmpty() && ((GameProfile)list4.get(0)).getId() != null) {
            return ((GameProfile)list4.get(0)).getId().toString();
        }
        return "";
    }
    
    public static boolean convertPlayers(final DedicatedServer uk) {
        final File file2 = getWorldPlayersDirectory(uk);
        ...
        try {
            ...
            final ProfileLookupCallback profileLookupCallback8 = (ProfileLookupCallback)new ProfileLookupCallback() {
                public void onProfileLookupSucceeded(final GameProfile gameProfile) {
                    uk.getProfileCache().add(gameProfile);
            ...
            lookupPlayers(uk, (Collection<String>)Lists.newArrayList((Object[])arr6), profileLookupCallback8);
        }
    ...
    private static File getWorldPlayersDirectory(final MinecraftServer minecraftServer) {
        final String string2 = minecraftServer.getLevelIdName();
