            ...
            for (int integer9 = 0; integer9 < list10.size(); ++integer9) {
                ...
                for (final int integer12 : stream14::iterator) {
                    if (416 == (integer12 & 0xFF) << 4) {
                        ...
                        list9.add(((Optional)type2.read(dynamic7.createMap((Map)map21)).getSecond()).orElseThrow(() -> new IllegalStateException("Could not parse newly created bed block entity.")));
                    }
                    ++integer11;
                }
            }
