...
public class BlockEntityBannerColorFix extends NamedEntityFix {
    ...
    public Dynamic<?> fixTag(Dynamic<?> dynamic) {
        ...
        dynamic = dynamic.update("Patterns", dynamic -> (Dynamic)DataFixUtils.orElse(dynamic.asStreamOpt().map(stream -> stream.map(dynamic -> dynamic.update("Color", dynamic -> dynamic.createInt(15 - dynamic.asInt(0))))).map(dynamic::createList), dynamic));
        return dynamic;
    }
