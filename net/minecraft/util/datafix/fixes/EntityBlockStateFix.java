...
public class EntityBlockStateFix extends DataFix {
    ...
    public TypeRewriteRule makeRule() {
        ...
        final Type<Pair<Either<Pair<String, Either<Integer, String>>, Unit>, Dynamic<?>>> type6 = (Type<Pair<Either<Pair<String, Either<Integer, String>>, Unit>, Dynamic<?>>>)DSL.and(DSL.optional((Type)DSL.field("inTile", DSL.named(References.BLOCK_NAME.typeName(), DSL.or(DSL.intType(), DSL.namespacedString())))), DSL.remainderType());
        final Function<Typed<?>, Typed<?>> function6 = (Function<Typed<?>, Typed<?>>)(typed -> typed.update(type6.finder(), DSL.remainderType(), Pair::getSecond));
        return this.fixTypeEverywhereTyped("EntityBlockStateFix", schema2.getType(References.ENTITY), schema3.getType(References.ENTITY), typed -> {
    ...
    private Typed<?> updateFallingBlock(final Typed<?> typed) {
        final Type<Either<Pair<String, Either<Integer, String>>, Unit>> type3 = (Type<Either<Pair<String, Either<Integer, String>>, Unit>>)DSL.optional((Type)DSL.field("Block", DSL.named(References.BLOCK_NAME.typeName(), DSL.or(DSL.intType(), DSL.namespacedString()))));
        final Type<Either<Pair<String, Dynamic<?>>, Unit>> type4 = (Type<Either<Pair<String, Dynamic<?>>, Unit>>)DSL.optional((Type)DSL.field("BlockState", DSL.named(References.BLOCK_STATE.typeName(), DSL.remainderType())));
        final Dynamic<?> dynamic5 = typed.get(DSL.remainderFinder());
        return typed.update(type3.finder(), (Type)type4, either -> {
            final int integer3 = (int)either.map(pair -> (Integer)((Either)pair.getSecond()).map(integer -> integer, EntityBlockStateFix::getBlockId), unit -> {
                final Optional<Number> optional3 = (Optional<Number>)dynamic5.get("TileID").asNumber();
            ...
            return Either.left(Pair.of((Object)References.BLOCK_STATE.typeName(), (Object)BlockStateData.getTag(integer3 << 4 | integer4)));
        }).set(DSL.remainderFinder(), dynamic5.remove("Data").remove("TileID").remove("Tile"));
    }
    
    private Typed<?> updateBlockToBlockState(final Typed<?> typed, final String string2, final String string3, final String string4) {
        final Type<Pair<String, Either<Integer, String>>> type6 = (Type<Pair<String, Either<Integer, String>>>)DSL.field(string2, DSL.named(References.BLOCK_NAME.typeName(), DSL.or(DSL.intType(), DSL.namespacedString())));
        final Type<Pair<String, Dynamic<?>>> type7 = (Type<Pair<String, Dynamic<?>>>)DSL.field(string4, DSL.named(References.BLOCK_STATE.typeName(), DSL.remainderType()));
        final Dynamic<?> dynamic8 = typed.getOrCreate(DSL.remainderFinder());
        return typed.update(type6.finder(), (Type)type7, pair -> {
            final int integer4 = (int)((Either)pair.getSecond()).map(integer -> integer, EntityBlockStateFix::getBlockId);
            final int integer5 = dynamic8.get(string3).asInt(0) & 0xF;
            return Pair.of(References.BLOCK_STATE.typeName(), BlockStateData.getTag(integer4 << 4 | integer5));
        }).set(DSL.remainderFinder(), dynamic8.remove(string3));
    }
    ...
    private Typed<?> updateEntity(final Typed<?> typed, final String string, final Function<Typed<?>, Typed<?>> function) {
        ...
        return typed.updateTyped(DSL.namedChoice(string, (Type)type5), (Type)type6, (Function)function);
    }
