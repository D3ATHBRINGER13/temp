    ...
    private <IS> TypeRewriteRule cap(final Type<IS> type) {
        ...
        return this.fixTypeEverywhereTyped("EntityEquipmentToArmorAndHandFix", this.getInputSchema().getType(References.ENTITY), this.getOutputSchema().getType(References.ENTITY), typed -> {
            ...
            if (optional9.isPresent()) {
                ...
                final Object object11 = ((Optional)type.read(dynamic8.emptyMap()).getSecond()).orElseThrow(() -> new IllegalStateException("Could not parse newly created empty itemstack."));
                if (!list10.isEmpty()) {
            ...
            final Optional<? extends Stream<? extends Dynamic<?>>> optional10 = dynamic8.get("DropChances").asStreamOpt();
            if (optional10.isPresent()) {
                ...
                if (!dynamic8.get("HandDropChances").get().isPresent()) {
                    final Dynamic<?> dynamic10 = dynamic8.emptyMap().merge(dynamic8.createFloat(float13)).merge(dynamic8.createFloat(0.0f));
                    dynamic8 = dynamic8.set("HandDropChances", (Dynamic)dynamic10);
                }
                if (!dynamic8.get("ArmorDropChances").get().isPresent()) {
                    final Dynamic<?> dynamic10 = dynamic8.emptyMap().merge(dynamic8.createFloat(((Dynamic)iterator12.next()).asFloat(0.0f))).merge(dynamic8.createFloat(((Dynamic)iterator12.next()).asFloat(0.0f))).merge(dynamic8.createFloat(((Dynamic)iterator12.next()).asFloat(0.0f))).merge(dynamic8.createFloat(((Dynamic)iterator12.next()).asFloat(0.0f)));
