...
public class IglooMetadataRemovalFix extends DataFix {
    ...
    private static <T> Dynamic<T> fixTag(final Dynamic<T> dynamic) {
        final boolean boolean2 = (boolean)dynamic.get("Children").asStreamOpt().map(stream -> stream.allMatch(IglooMetadataRemovalFix::isIglooPiece)).orElse(false);
        if (boolean2) {
    ...
    private static <T> Dynamic<T> removeIglooPieces(final Dynamic<T> dynamic) {
        return (Dynamic<T>)dynamic.asStreamOpt().map(stream -> stream.filter(dynamic -> !isIglooPiece(dynamic))).map(dynamic::createList).orElse(dynamic);
    }
