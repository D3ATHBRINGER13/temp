...
public class ItemStackEnchantmentNamesFix extends DataFix {
    ...
    private Dynamic<?> fixTag(Dynamic<?> dynamic) {
        final Optional<Dynamic<?>> optional3 = (Optional<Dynamic<?>>)dynamic.get("ench").asStreamOpt().map(stream -> stream.map(dynamic -> dynamic.set("id", dynamic.createString((String)ItemStackEnchantmentNamesFix.MAP.getOrDefault(dynamic.get("id").asInt(0), "null"))))).map(dynamic::createList);
        if (optional3.isPresent()) {
            dynamic = dynamic.remove("ench").set("Enchantments", (Dynamic)optional3.get());
        }
        return dynamic.update("StoredEnchantments", dynamic -> (Dynamic)DataFixUtils.orElse(dynamic.asStreamOpt().map(stream -> stream.map(dynamic -> dynamic.set("id", dynamic.createString((String)ItemStackEnchantmentNamesFix.MAP.getOrDefault(dynamic.get("id").asInt(0), "null"))))).map(dynamic::createList), dynamic));
    }
