...
public class MobSpawnerEntityIdentifiersFix extends DataFix {
    ...
    private Dynamic<?> fix(Dynamic<?> dynamic) {
        ...
        if (optional4.isPresent()) {
            dynamic = dynamic.set("SpawnPotentials", dynamic.createList(((Stream)optional4.get()).map(dynamic -> {
                final Optional<String> optional2 = (Optional<String>)dynamic.get("Type").asString();
                if (optional2.isPresent()) {
                    final Dynamic<?> dynamic2 = ((Dynamic)DataFixUtils.orElse(dynamic.get("Properties").get(), dynamic.emptyMap())).set("id", dynamic.createString((String)optional2.get()));
    ...
    public TypeRewriteRule makeRule() {
        ...
        return this.fixTypeEverywhereTyped("MobSpawnerEntityIdentifiersFix", this.getInputSchema().getType(References.UNTAGGED_SPAWNER), (Type)type2, typed -> {
            ...
            final Pair<?, ? extends Optional<? extends Typed<?>>> pair5 = type2.readTyped((Dynamic)this.fix(dynamic4));
