...
public class OptionsKeyLwjgl3Fix extends DataFix {
    ...
    public TypeRewriteRule makeRule() {
        return this.fixTypeEverywhereTyped("OptionsKeyLwjgl3Fix", this.getInputSchema().getType(References.OPTIONS), typed -> typed.update(DSL.remainderFinder(), dynamic -> (Dynamic)dynamic.getMapValues().map(map -> dynamic.createMap((Map)map.entrySet().stream().map(entry -> {
            ...
            if (integer2 < 0) {
                ...
                return Pair.of(entry.getKey(), ((Dynamic)entry.getValue()).createString(string4));
            }
            final String string5 = (String)OptionsKeyLwjgl3Fix.MAP.getOrDefault(integer2, "key.unknown");
