...
public class DifficultyInstance {
    ...
    public DifficultyInstance(final Difficulty ahg, final long long2, final long long3, final float float4) {
        this.base = ahg;
        this.effectiveDifficulty = this.calculateDifficulty(ahg, long2, long3, float4);
    }
    ...
    private float calculateDifficulty(final Difficulty ahg, final long long2, final long long3, final float float4) {
        if (ahg == Difficulty.PEACEFUL) {
            return 0.0f;
        }
        final boolean boolean8 = ahg == Difficulty.HARD;
        float float5 = 0.75f;
        ...
        if (ahg == Difficulty.EASY) {
            float7 *= 0.5f;
        }
        float5 += float7;
        return ahg.getId() * float5;
    }
}
