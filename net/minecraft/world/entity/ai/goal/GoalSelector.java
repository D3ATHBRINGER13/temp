package net.minecraft.world.entity.ai.goal;

import org.apache.logging.log4j.LogManager;
import java.util.stream.Stream;
import com.google.common.collect.Sets;
import java.util.EnumMap;
import java.util.EnumSet;
import net.minecraft.util.profiling.ProfilerFiller;
import java.util.Set;
import java.util.Map;
import org.apache.logging.log4j.Logger;

public class GoalSelector {
    private static final Logger LOGGER;
    private static final WrappedGoal NO_GOAL;
    private final Map<Goal.Flag, WrappedGoal> lockedFlags;
    private final Set<WrappedGoal> availableGoals;
    private final ProfilerFiller profiler;
    private final EnumSet<Goal.Flag> disabledFlags;
    private int newGoalRate;
    
    public GoalSelector(final ProfilerFiller agn) {
        this.lockedFlags = (Map<Goal.Flag, WrappedGoal>)new EnumMap((Class)Goal.Flag.class);
        this.availableGoals = (Set<WrappedGoal>)Sets.newLinkedHashSet();
        this.disabledFlags = (EnumSet<Goal.Flag>)EnumSet.noneOf((Class)Goal.Flag.class);
        this.newGoalRate = 3;
        this.profiler = agn;
    }
    
    public void addGoal(final int integer, final Goal ane) {
        this.availableGoals.add(new WrappedGoal(integer, ane));
    }
    
    public void removeGoal(final Goal ane) {
        this.availableGoals.stream().filter(aot -> aot.getGoal() == ane).filter(WrappedGoal::isRunning).forEach(WrappedGoal::stop);
        this.availableGoals.removeIf(aot -> aot.getGoal() == ane);
    }
    
    public void tick() {
        this.profiler.push("goalCleanup");
        this.getRunningGoals().filter(aot -> !aot.isRunning() || aot.getFlags().stream().anyMatch(this.disabledFlags::contains) || !aot.canContinueToUse()).forEach(Goal::stop);
        this.lockedFlags.forEach((a, aot) -> {
            if (!aot.isRunning()) {
                this.lockedFlags.remove(a);
            }
        });
        this.profiler.pop();
        this.profiler.push("goalUpdate");
        this.availableGoals.stream().filter(aot -> !aot.isRunning()).filter(aot -> aot.getFlags().stream().noneMatch(this.disabledFlags::contains)).filter(aot -> aot.getFlags().stream().allMatch(a -> ((WrappedGoal)this.lockedFlags.getOrDefault(a, GoalSelector.NO_GOAL)).canBeReplacedBy(aot))).filter(WrappedGoal::canUse).forEach(aot -> {
            aot.getFlags().forEach(a -> {
                final WrappedGoal aot2 = (WrappedGoal)this.lockedFlags.getOrDefault(a, GoalSelector.NO_GOAL);
                aot2.stop();
                this.lockedFlags.put(a, aot);
            });
            aot.start();
        });
        this.profiler.pop();
        this.profiler.push("goalTick");
        this.getRunningGoals().forEach(WrappedGoal::tick);
        this.profiler.pop();
    }
    
    public Stream<WrappedGoal> getRunningGoals() {
        return (Stream<WrappedGoal>)this.availableGoals.stream().filter(WrappedGoal::isRunning);
    }
    
    public void disableControlFlag(final Goal.Flag a) {
        this.disabledFlags.add(a);
    }
    
    public void enableControlFlag(final Goal.Flag a) {
        this.disabledFlags.remove(a);
    }
    
    public void setControlFlag(final Goal.Flag a, final boolean boolean2) {
        if (boolean2) {
            this.enableControlFlag(a);
        }
        else {
            this.disableControlFlag(a);
        }
    }
    
    static {
        LOGGER = LogManager.getLogger();
        NO_GOAL = new WrappedGoal(Integer.MAX_VALUE, new Goal() {
            @Override
            public boolean canUse() {
                return false;
            }
        }) {
            @Override
            public boolean isRunning() {
                return false;
            }
        };
    }
}
