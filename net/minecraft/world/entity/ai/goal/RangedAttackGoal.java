...
public class RangedAttackGoal extends Goal {
    ...
    public RangedAttackGoal(final RangedAttackMob auy, final double double2, final int integer, final float float4) {
        this(auy, double2, integer, integer, float4);
    }
    
    public RangedAttackGoal(final RangedAttackMob auy, final double double2, final int integer3, final int integer4, final float float5) {
        this.attackTime = -1;
        if (!(auy instanceof LivingEntity)) {
            throw new IllegalArgumentException("ArrowAttackGoal requires Mob implements RangedAttackMob");
        }
        this.rangedAttackMob = auy;
        this.mob = (Mob)auy;
    ...
    @Override
    public void tick() {
        final double double2 = this.mob.distanceToSqr(this.target.x, this.target.getBoundingBox().minY, this.target.z);
