...
public class RangedBowAttackGoal<T extends Monster> extends Goal {
    ...
    public RangedBowAttackGoal(final T aus, final double double2, final int integer, final float float4) {
        this.attackTime = -1;
        this.strafingTime = -1;
        this.mob = aus;
    ...
    public void tick() {
        ...
        final double double3 = ((Entity)this.mob).distanceToSqr(aix2.x, aix2.getBoundingBox().minY, aix2.z);
        final boolean boolean5 = ((Mob)this.mob).getSensing().canSee(aix2);
        ...
        if (double3 > this.attackRadiusSqr || this.seeTime < 20) {
            ((Mob)this.mob).getNavigation().moveTo(aix2, this.speedModifier);
            this.strafingTime = -1;
        }
        else {
        ...
        if (this.strafingTime > -1) {
            ...
            ((Mob)this.mob).lookAt(aix2, 30.0f, 30.0f);
        }
        else {
        ...
        if (((LivingEntity)this.mob).isUsingItem()) {
            if (!boolean5 && this.seeTime < -60) {
                ...
                if (integer7 >= 20) {
                    ((LivingEntity)this.mob).stopUsingItem();
                    ((RangedAttackMob)this.mob).performRangedAttack(aix2, BowItem.getPowerForTime(integer7));
