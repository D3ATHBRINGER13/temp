...
public class RunAroundLikeCrazyGoal extends Goal {
    ...
    public RunAroundLikeCrazyGoal(final AbstractHorse asb, final double double2) {
        this.horse = asb;
    ...
    public boolean canUse() {
        ...
        final Vec3 csi2 = RandomPos.getPos(this.horse, 5, 4);
        if (csi2 == null) {
            return false;
        }
        this.posX = csi2.x;
    ...
    public void tick() {
        if (!this.horse.isTamed() && this.horse.getRandom().nextInt(50) == 0) {
            final Entity aio2 = (Entity)this.horse.getPassengers().get(0);
            if (aio2 == null) {
                return;
            }
            if (aio2 instanceof Player) {
                ...
                if (integer4 > 0 && this.horse.getRandom().nextInt(integer4) < integer3) {
                    this.horse.tameWithName((Player)aio2);
                    return;
                }
