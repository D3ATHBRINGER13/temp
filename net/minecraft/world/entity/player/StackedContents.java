...
public class StackedContents {
    ...
    public void accountSimpleStack(final ItemStack bcj) {
        if (!bcj.isDamaged() && !bcj.isEnchanted() && !bcj.hasCustomHoverName()) {
            this.accountStack(bcj);
        }
    }
    
    public void accountStack(final ItemStack bcj) {
        this.accountStack(bcj, 64);
    }
    
    public void accountStack(final ItemStack bcj, final int integer) {
        if (!bcj.isEmpty()) {
            final int integer2 = getStackingIndex(bcj);
            final int integer3 = Math.min(integer, bcj.getCount());
            this.put(integer2, integer3);
        }
    }
    
    public static int getStackingIndex(final ItemStack bcj) {
        return Registry.ITEM.getId(bcj.getItem());
    }
    ...
    public boolean canCraft(final Recipe<?> ber, @Nullable final IntList intList) {
        return this.canCraft(ber, intList, 1);
    }
    
    public boolean canCraft(final Recipe<?> ber, @Nullable final IntList intList, final int integer) {
        return new RecipePicker(ber).tryPick(integer, intList);
    }
    
    public int getBiggestCraftableStack(final Recipe<?> ber, @Nullable final IntList intList) {
        return this.getBiggestCraftableStack(ber, Integer.MAX_VALUE, intList);
    }
    
    public int getBiggestCraftableStack(final Recipe<?> ber, final int integer, @Nullable final IntList intList) {
        return new RecipePicker(ber).tryPickAll(integer, intList);
    }
    ...
    class RecipePicker {
        ...
        public RecipePicker(final Recipe<?> ber) {
            this.ingredients = (List<Ingredient>)Lists.newArrayList();
            this.path = (IntList)new IntArrayList();
            this.recipe = ber;
            this.ingredients.addAll((Collection)ber.getIngredients());
        ...
        private int[] getUniqueAvailableIngredientItems() {
            ...
            for (final Ingredient beo4 : this.ingredients) {
                intCollection2.addAll((IntCollection)beo4.getStackingIds());
            }
        ...
        private int getMinIngredientCount() {
            ...
            for (final Ingredient beo4 : this.ingredients) {
                int integer3 = 0;
                for (final int integer4 : beo4.getStackingIds()) {
