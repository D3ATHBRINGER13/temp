...
public enum DyeColor implements StringRepresentable {
    ...
    private DyeColor(final int integer3, final String string4, final int integer5, final MaterialColor clp, final int integer7, final int integer8) {
        this.id = integer3;
        this.name = string4;
    ...
    public static DyeColor byName(final String string, final DyeColor bbg) {
        for (final DyeColor bbg2 : values()) {
            if (bbg2.name.equals(string)) {
                return bbg2;
            }
        }
        return bbg;
    ...
    static {
        ...
        BY_FIREWORK_COLOR = new Int2ObjectOpenHashMap((Map)Arrays.stream((Object[])values()).collect(Collectors.toMap(bbg -> bbg.fireworkColor, bbg -> bbg)));
    }
}
