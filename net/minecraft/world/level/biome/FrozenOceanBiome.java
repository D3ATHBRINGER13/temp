...
public final class FrozenOceanBiome extends Biome {
    ...
    public FrozenOceanBiome() {
        super(new BiomeBuilder().<SurfaceBuilderBaseConfiguration>surfaceBuilder(SurfaceBuilder.FROZEN_OCEAN, SurfaceBuilder.CONFIG_GRASS).precipitation(Precipitation.SNOW).biomeCategory(BiomeCategory.OCEAN).depth(-1.0f).scale(0.1f).temperature(0.0f).downfall(0.5f).waterColor(3750089).waterFogColor(329011).parent(null));
    ...
    protected float getTemperatureNoCache(final BlockPos ew) {
        ...
        final double double4 = FrozenOceanBiome.FROZEN_TEMPERATURE_NOISE.getValue(ew.getX() * 0.05, ew.getZ() * 0.05);
        final double double5 = FrozenOceanBiome.BIOME_INFO_NOISE.getValue(ew.getX() * 0.2, ew.getZ() * 0.2);
        final double double6 = double4 + double5;
        if (double6 < 0.3) {
            final double double7 = FrozenOceanBiome.BIOME_INFO_NOISE.getValue(ew.getX() * 0.09, ew.getZ() * 0.09);
            if (double7 < 0.8) {
                float3 = 0.2f;
            }
        }
        if (ew.getY() > 64) {
            final float float4 = (float)(FrozenOceanBiome.TEMPERATURE_NOISE.getValue(ew.getX() / 8.0f, ew.getZ() / 8.0f) * 4.0);
