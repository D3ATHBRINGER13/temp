...
public final class SavannaBiome extends Biome {
    protected SavannaBiome() {
        super(new BiomeBuilder().<SurfaceBuilderBaseConfiguration>surfaceBuilder(SurfaceBuilder.DEFAULT, SurfaceBuilder.CONFIG_GRASS).precipitation(Precipitation.NONE).biomeCategory(BiomeCategory.SAVANNA).depth(0.125f).scale(0.05f).temperature(1.2f).downfall(0.0f).waterColor(4159204).waterFogColor(329011).parent(null));
        this.<VillageConfiguration>addStructureStart(Feature.VILLAGE, new VillageConfiguration("village/savanna/town_centers", 6));
