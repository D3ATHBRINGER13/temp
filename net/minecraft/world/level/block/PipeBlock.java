...
public class PipeBlock extends Block {
    ...
    private VoxelShape[] makeShapes(final float float1) {
        ...
        final VoxelShape ctc5 = Block.box(float2 * 16.0f, float2 * 16.0f, float2 * 16.0f, float3 * 16.0f, float3 * 16.0f, float3 * 16.0f);
        ...
        for (int integer7 = 0; integer7 < PipeBlock.DIRECTIONS.length; ++integer7) {
            final Direction fb8 = PipeBlock.DIRECTIONS[integer7];
            arr6[integer7] = Shapes.box(0.5 + Math.min((double)(-float1), fb8.getStepX() * 0.5), 0.5 + Math.min((double)(-float1), fb8.getStepY() * 0.5), 0.5 + Math.min((double)(-float1), fb8.getStepZ() * 0.5), 0.5 + Math.max((double)float1, fb8.getStepX() * 0.5), 0.5 + Math.max((double)float1, fb8.getStepY() * 0.5), 0.5 + Math.max((double)float1, fb8.getStepZ() * 0.5));
        }
        final VoxelShape[] arr7 = new VoxelShape[64];
        for (int integer8 = 0; integer8 < 64; ++integer8) {
            VoxelShape ctc6 = ctc5;
            for (int integer9 = 0; integer9 < PipeBlock.DIRECTIONS.length; ++integer9) {
                if ((integer8 & 1 << integer9) != 0x0) {
                    ctc6 = Shapes.or(ctc6, arr6[integer9]);
                }
            }
            arr7[integer8] = ctc6;
        }
    ...
    protected int getAABBIndex(final BlockState bvt) {
        ...
        for (int integer4 = 0; integer4 < PipeBlock.DIRECTIONS.length; ++integer4) {
            if (bvt.<Boolean>getValue((Property<Boolean>)PipeBlock.PROPERTY_BY_DIRECTION.get(PipeBlock.DIRECTIONS[integer4]))) {
                integer3 |= 1 << integer4;
            }
        }
        return integer3;
