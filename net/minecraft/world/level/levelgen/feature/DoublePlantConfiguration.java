...
public class DoublePlantConfiguration implements FeatureConfiguration {
    ...
    public DoublePlantConfiguration(final BlockState bvt) {
        this.state = bvt;
    }
    ...
    public static <T> DoublePlantConfiguration deserialize(final Dynamic<T> dynamic) {
        final BlockState bvt2 = (BlockState)dynamic.get("state").map(BlockState::deserialize).orElse(Blocks.AIR.defaultBlockState());
        return new DoublePlantConfiguration(bvt2);
    }
}
