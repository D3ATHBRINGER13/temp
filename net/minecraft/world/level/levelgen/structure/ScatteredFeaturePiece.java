...
public abstract class ScatteredFeaturePiece extends StructurePiece {
    ...
    protected ScatteredFeaturePiece(final StructurePieceType cev, final Random random, final int integer3, final int integer4, final int integer5, final int integer6, final int integer7, final int integer8) {
        super(cev, 0);
    ...
    protected ScatteredFeaturePiece(final StructurePieceType cev, final CompoundTag id) {
        super(cev, id);
        this.heightPosition = -1;
        this.width = id.getInt("Width");
        this.height = id.getInt("Height");
        this.depth = id.getInt("Depth");
        this.heightPosition = id.getInt("HPos");
    }
    
    @Override
    protected void addAdditionalSaveData(final CompoundTag id) {
        id.putInt("Width", this.width);
        id.putInt("Height", this.height);
        id.putInt("Depth", this.depth);
        id.putInt("HPos", this.heightPosition);
    }
    
    protected boolean updateAverageGroundHeight(final LevelAccessor bhs, final BoundingBox cic, final int integer) {
        ...
        for (int integer4 = this.boundingBox.z0; integer4 <= this.boundingBox.z1; ++integer4) {
            for (int integer5 = this.boundingBox.x0; integer5 <= this.boundingBox.x1; ++integer5) {
                ...
                if (cic.isInside(a7)) {
                    integer2 += bhs.getHeightmapPos(Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, a7).getY();
                    ++integer3;
                }
            }
        }
