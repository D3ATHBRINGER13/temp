...
public class BuiltInLootTables {
    ...
    private static ResourceLocation register(final String string) {
        return register(new ResourceLocation(string));
    }
    
    private static ResourceLocation register(final ResourceLocation qv) {
        if (BuiltInLootTables.LOCATIONS.add(qv)) {
            return qv;
        }
        throw new IllegalArgumentException(new StringBuilder().append(qv).append(" is already a registered built-in loot table").toString());
    }
    ...
    static {
        LOCATIONS = (Set)Sets.newHashSet();
        IMMUTABLE_LOCATIONS = Collections.unmodifiableSet((Set)BuiltInLootTables.LOCATIONS);
