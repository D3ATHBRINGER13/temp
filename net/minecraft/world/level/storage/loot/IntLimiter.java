...
public class IntLimiter implements IntUnaryOperator {
    ...
    public static class Serializer implements JsonDeserializer<IntLimiter>, JsonSerializer<IntLimiter> {
        ...
        public JsonElement serialize(final IntLimiter cox, final Type type, final JsonSerializationContext jsonSerializationContext) {
            final JsonObject jsonObject5 = new JsonObject();
            if (cox.max != null) {
                jsonObject5.addProperty("max", (Number)cox.max);
            }
            if (cox.min != null) {
                jsonObject5.addProperty("min", (Number)cox.min);
            }
