...
public class RandomValueBounds implements RandomIntGenerator {
    ...
    public static class Serializer implements JsonDeserializer<RandomValueBounds>, JsonSerializer<RandomValueBounds> {
        ...
        public JsonElement serialize(final RandomValueBounds cpg, final Type type, final JsonSerializationContext jsonSerializationContext) {
            if (cpg.min == cpg.max) {
                return (JsonElement)new JsonPrimitive((Number)cpg.min);
            }
            final JsonObject jsonObject5 = new JsonObject();
            jsonObject5.addProperty("min", (Number)cpg.min);
            jsonObject5.addProperty("max", (Number)cpg.max);
