...
public class CopyNbtFunction extends LootItemConditionalFunction {
    ...
    public ItemStack run(final ItemStack bcj, final LootContext coy) {
        final Tag iu4 = (Tag)this.source.getter.apply(coy);
        if (iu4 != null) {
            this.operations.forEach(b -> b.apply((Supplier<Tag>)bcj::getOrCreateTag, iu4));
        }
        return bcj;
    }
    ...
    static {
        ...
        BLOCK_ENTITY_GETTER = (btw -> btw.save(new CompoundTag()));
    }
    ...
    static class CopyOperation {
        ...
        public void apply(final Supplier<Tag> supplier, final Tag iu) {
            try {
                final List<Tag> list4 = this.sourcePath.get(iu);
                if (!list4.isEmpty()) {
    ...
    public enum MergeStrategy {
        REPLACE("replace") {
            @Override
            public void merge(final Tag iu, final NbtPathArgument.NbtPath h, final List<Tag> list) throws CommandSyntaxException {
                h.set(iu, (Supplier<Tag>)(Tag)Iterables.getLast((Iterable)list)::copy);
            }
        }, 
        APPEND("append") {
            @Override
            public void merge(final Tag iu, final NbtPathArgument.NbtPath h, final List<Tag> list) throws CommandSyntaxException {
                final List<Tag> list2 = h.getOrCreate(iu, (Supplier<Tag>)ListTag::new);
                list2.forEach(iu -> {
                    if (iu instanceof ListTag) {
                        list.forEach(iu2 -> ((ListTag)iu).add(iu2.copy()));
                    }
                });
            }
        }, 
        MERGE("merge") {
            @Override
            public void merge(final Tag iu, final NbtPathArgument.NbtPath h, final List<Tag> list) throws CommandSyntaxException {
                final List<Tag> list2 = h.getOrCreate(iu, (Supplier<Tag>)CompoundTag::new);
                list2.forEach(iu -> {
                    if (iu instanceof CompoundTag) {
                        list.forEach(iu2 -> {
                            if (iu2 instanceof CompoundTag) {
                                ((CompoundTag)iu).merge((CompoundTag)iu2);
                            }
                        });
                    }
                });
        ...
        public abstract void merge(final Tag iu, final NbtPathArgument.NbtPath h, final List<Tag> list) throws CommandSyntaxException;
    ...
    public enum DataSource {
        ...
        private <T> DataSource(final String string3, final LootContextParam<T> cqw, final Function<? super T, Tag> function) {
            this.name = string3;
            this.param = cqw;
            this.getter = (Function<LootContext, Tag>)(coy -> {
                final Object object4 = coy.getParamOrNull((LootContextParam<Object>)cqw);
    ...
    public static class Serializer extends LootItemConditionalFunction.Serializer<CopyNbtFunction> {
        ...
        @Override
        public void serialize(final JsonObject jsonObject, final CopyNbtFunction cpz, final JsonSerializationContext jsonSerializationContext) {
            super.serialize(jsonObject, cpz, jsonSerializationContext);
            jsonObject.addProperty("source", cpz.source.name);
            final JsonArray jsonArray5 = new JsonArray();
            cpz.operations.stream().map(CopyOperation::toJson).forEach(jsonArray5::add);
