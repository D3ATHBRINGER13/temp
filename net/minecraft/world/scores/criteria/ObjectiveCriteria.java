...
public class ObjectiveCriteria {
    ...
    public static Optional<ObjectiveCriteria> byName(final String string) {
        ...
        return (Optional<ObjectiveCriteria>)Registry.STAT_TYPE.getOptional(ResourceLocation.of(string.substring(0, integer2), '.')).flatMap(yx -> ObjectiveCriteria.getStat((StatType<Object>)yx, ResourceLocation.of(string.substring(integer2 + 1), '.')));
    }
    
    private static <T> Optional<ObjectiveCriteria> getStat(final StatType<T> yx, final ResourceLocation qv) {
        return (Optional<ObjectiveCriteria>)yx.getRegistry().getOptional(qv).map(yx::get);
    }
